...
describe("dead-end detection", () => {
  it("is NOT a dead end when a legal (even unproductive) move exists", () => {
    ...
    expect(isDeadEnd(s, false)).toBe(false);
  });
  ...
  it("is a dead end when stock/waste empty and no legal move", () => {
    ...
    expect(isDeadEnd(s, false)).toBe(true);
  });

  it("with unplayable stock, only dead once the stock has cycled without progress", () => {
    const s = emptyState();
    s.stock = [down("4", "clubs"), down("6", "hearts")];
    s.waste = [up("9", "spades")];
    s.tableau[0] = [up("5", "spades")];
    s.tableau[1] = [up("3", "clubs")];
    expect(isDeadEnd(s, false)).toBe(false); // keep drawing
    expect(isDeadEnd(s, true)).toBe(true); // cycled, no progress
  });
